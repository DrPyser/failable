...
class datatype(type):
    ...
    def __new__(cls, name, bases, attrs, cached=False, maxsize=100):
        if "__fields__" in attrs:
            fields = attrs["__fields__"] = tuple(attrs["__fields__"])
        elif "__annotations__" in attrs:
            attrs["__fields__"] = fields = tuple(attrs["__annotations__"].keys())
        else:
            attrs["__fields__"] = fields = ()
            
        #attrs["__slots__"] = attrs["__fields__"]

        for i, field in enumerate(fields):
            attrs[field] = property(itemgetter(i))
        if cached:
            attrs["__cache__"] = ct.LRUCache(maxsize=(1 if len(fields) == 0 else maxsize))
            attrs["__cached__"] = True
        else:
            attrs["__cached__"] = False
        return super().__new__(cls, name, bases, attrs)

    def __init__(cls, name, bases, attrs, functoid=True, **kwargs):
...
class data(tuple, metaclass=datatype):
    def __new__(cls, *args, **kwargs):
        if len(kwargs) + len(args) != len(cls.__fields__):
            ...
            if cls.__cached__:
                cached = cls.__cache__.get(values, None)
                if cached is None:
                    new = cls.__cache__[values] = tuple.__new__(cls, values)
                    return new
                else:
                    return cached
            else:
    ...
    #     raise Exception("You cannot mutate instance of immutable class {}.".format(type(self).__name__))
